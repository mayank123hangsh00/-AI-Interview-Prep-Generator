...
export function allocateSchedule(questions, requirements, daysAvailable) {
    ...
    const scored = questions.map((q) => {
        ...
        const coversMust = q.requirement_ids.some((id) => mustRequirementIds.has(id));
        if (coversMust)
            score += 10;
    ...
    for (const sq of scored) {
        ...
        if (currentMinutes + sq.minutes > minutesPerDay * 1.2 && // Allow 20% overflow
 ...
 */
function buildDay(dayNum, questions, totalMinutes) {
 ...
 */
function getReviewQuestions(existingDays, currentDayNum) {
    const review = [];
    const maxReview = 5;
    // Take questions from the earliest days first
    for (const day of existingDays) {
        for (const qId of day.question_ids) {
            if (review.length >= maxReview)
                break;
            review.push(qId);
        }
        if (review.length >= maxReview)
...
//# sourceMappingURL=scheduleAllocator.js.map