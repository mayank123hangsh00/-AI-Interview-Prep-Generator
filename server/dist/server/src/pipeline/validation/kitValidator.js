...
export function validateKit(kit) {
    ...
    if (kit.schedule) {
        if (kit.schedule.days.length !== kit.schedule.days_available) {
            errors.push(`Schedule: days_available is ${kit.schedule.days_available} but has ${kit.schedule.days.length} days`);
        }
    }
    // 4. Unique IDs
 ...
 */
export function fixKit(kit) {
    const fixed = JSON.parse(JSON.stringify(kit));
    ...
    for (const q of fixed.questions) {
        ...
        if (q.difficulty > 3)
            q.difficulty = 3;
        q.difficulty = Math.round(q.difficulty);
    }
    ...
    const validReqIds = new Set(fixed.role.requirements.map((r) => r.id));
    for (const q of fixed.questions) {
    ...
    const validQIds = new Set(fixed.questions.map((q) => q.id));
    for (const day of fixed.schedule.days) {
        day.question_ids = day.question_ids.filter((id) => validQIds.has(id));
    }
...
//# sourceMappingURL=kitValidator.js.map