...
export async function runPipeline(options) {
    ...
    const companyBrief = await generateCompanyBrief(crawlResult.pages, discussionResult, companyUrl);
    updateStep(5, 'done');
    ...
    if (crawlResult.hiringPageFound) {
        ...
        const hiringPages = crawlResult.pages.filter((p) => p.content.toLowerCase().includes('interview') ||
    ...
    if (companyBrief.hiring_process) {
        hiringContext += '\n' + companyBrief.hiring_process;
    }
    let allQuestions = await generateAllQuestions(extractedRole.requirements, hiringContext, extractedRole.responsibilities);
