 ...
 */
export async function createBatchKits(req, res, next) {
    try {
 ...
 */
export async function getKitProgress(req, res, next) {
    try {
        const kitId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
...
export async function updateQuestion(req, res, next) {
    try {
        ...
        if (!kit)
            throw createError(404, 'Kit or question not found', 'NOT_FOUND');
        res.json({ kit });
    }
    catch (error) {
        next(error);
...
export async function addQuestion(req, res, next) {
    try {
        ...
        if (!kit)
            throw createError(404, 'Kit not found', 'NOT_FOUND');
        res.json({ kit });
    }
    catch (error) {
        next(error);
...
export async function updateFlashcard(req, res, next) {
    try {
        ...
        if (!kit)
            throw createError(404, 'Kit or flashcard not found', 'NOT_FOUND');
        res.json({ kit });
    }
    catch (error) {
        next(error);
 ...
 */
export async function updateCompanyBrief(req, res, next) {
    try {
        const kitId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
 ...
 */
export async function updatePracticeState(req, res, next) {
    try {
        const kitId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
