...
async function callGroqAPI(
  ...
  if (!apiKey || apiKey === 'your_groq_api_key_here') throw new Error('GROQ_API_KEY is required for Groq provider');
...
async function callGeminiAPI(
  ...
  const response: GenerateContentResult = await generativeModel.generateContent(fullPrompt);
...
export async function callLLM<T = any>(
  ...
  const result = await retryWithBackoff<LLMResponse<T>>(
    async () => {
