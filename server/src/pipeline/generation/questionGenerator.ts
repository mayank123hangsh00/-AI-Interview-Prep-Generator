...
export async function generateQuestionsForCategory(
  ...
  const response = await callLLM<Question[]>(
    QUESTION_GEN_SYSTEM(category, hiringContext),
    QUESTION_GEN_USER(requirements, startId, category),
...
export async function generateAllQuestions(
  ...
  if (technicalReqs.length > 0) {
    const techQuestions = await generateQuestionsForCategory(
      'technical', technicalReqs, hiringContext, nextId
    );
  ...
  if (behaviouralReqs.length > 0) {
    const behavQuestions = await generateQuestionsForCategory(
      'behavioural', behaviouralReqs, hiringContext, nextId
    );
  ...
  const hasSeniorContext =
    seniorityIndicators.includes('architect') ||
  ...
  if (hasSeniorContext && technicalReqs.length > 0) {
    ...
    const designQuestions = await generateQuestionsForCategory(
      'system-design', designReqs, hiringContext, nextId
    );
  ...
  if (fitReqs.length > 0) {
    const fitQuestions = await generateQuestionsForCategory(
      'company-fit', fitReqs, hiringContext, nextId
    );
...
export async function generateGapFillingQuestions(
  ...
  const response = await callLLM<Question[]>(
    GAP_FILL_SYSTEM,
    GAP_FILL_USER(uncoveredRequirements, startId),
