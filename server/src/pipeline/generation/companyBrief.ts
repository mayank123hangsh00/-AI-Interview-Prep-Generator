...
export async function generateCompanyBrief(
  ...
  const response = await callLLM<BriefGenerationResult>(
  ...
  if (response.data.hiring_process) {
    (brief as any).hiring_process = response.data.hiring_process;
  }
  if (response.data.culture_notes) {
    (brief as any).culture_notes = response.data.culture_notes;
  }
